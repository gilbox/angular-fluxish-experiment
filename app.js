...
app.factory('routeStore', function ($rootScope, appStore) {
  return {
    name: 'routeStore',
    actions: {
...
app.factory('actionCreator', function ($http, appStore, routeStore, $rootScope, localCache) {
  var searchParams = {
  ...
  function loadPhotos() {
    ...
    async.map(tags, photoSearch, function (e,r) {
      ...
      appStore.loadPhotosSuccess(r.concat(cachedPhotos));
  ...
  return {
    initFaces: function() {
      ...
      if (localCache.allFaces) {
        ...
        appStore.initFacesSuccess(localCache.allFaces);
        loadPhotos();
      } else {
        $http.get(url)
          .success(function(data) {
            localCache.allFaces = data;
            appStore.initFacesSuccess(data);
            loadPhotos();
          })
          .error(function(data) {
            console.log("error:", data);
            appStore.initFacesError(data);
          });
      }
    },
    selectPhoto: function(photo) {
      appStore.selectPhoto(photo);
      routeStore.selectPhoto(photo);
      loadPhotos();
    }
  };
});
